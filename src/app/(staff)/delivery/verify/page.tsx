...
import { Input } from "@/components/ui/input";
import { Button } from "@/components/ui/button";
import { Loader2, CheckCircle2, Truck, ShieldCheck, QrCode, X, AlertCircle } from "lucide-react";
import axios from "axios";
import { toast } from "sonner";
...
export default function DeliveryVerifyPage() {
  ...
  useEffect(() => {
    ...
    const startScanner = async () => {
      try {
        // Use the core engine for direct control
    ...
    // Delay ensures the "reader" ID is rendered in the DOM
    ...
    <div className="min-h-screen bg-[#141F2D] flex items-center justify-center p-4 relative overflow-hidden">
      <Card className="bg-[#1D2B3F]/40 backdrop-blur-xl border-[#EFA765]/20 max-w-md w-full rounded-[2.5rem] shadow-2xl relative overflow-hidden">
        ...
        {isScanning && (
          <div className="absolute inset-0 z-50 bg-[#141F2D] flex flex-col p-8">
            ...
            <div className="relative mx-auto w-full max-w-[280px]">
              {/* The "reader" div must be present when startScanner runs */}
            ...
            <div className="mt-8 flex flex-col items-center gap-4 px-4 text-center">
              ...
              <div className="flex items-start gap-2 text-[10px] text-yellow-500/60 bg-yellow-500/5 p-3 rounded-xl border border-yellow-500/10">
                 <AlertCircle className="h-4 w-4 shrink-0" />
                 <p>Black screen? Tap the lock 🔒 icon in your browser address bar and set Camera to &quot;Allow&quot;.</p>
              </div>
            </div>
          </div>
        )}
        ...
        <CardHeader className="text-center pt-12">
          ...
          <CardTitle className="text-3xl font-black text-[#EFA765]">Staff Portal</CardTitle>
          <p className="text-gray-400 text-[10px] font-bold uppercase tracking-[0.3em] mt-2">Verification System</p>
        </CardHeader>
        ...
        <CardContent className="px-8 pb-12 space-y-8">
          <Button 
            type="button"
            onClick={() => setIsScanning(true)}
            className="w-full bg-[#EFA765]/5 border border-[#EFA765]/30 text-[#EFA765] hover:bg-[#EFA765] hover:text-[#141F2D] h-20 rounded-3xl flex items-center justify-center gap-4 transition-all font-black group"
          >
            <QrCode className="h-6 w-6" /> 
            <span className="text-sm tracking-wide">SCAN RECEIPT</span>
          </Button>
          ...
          <div className="relative flex items-center">
            <div className="flex-grow border-t border-white/5"></div>
            <span className="mx-4 text-[10px] font-bold text-white/20 uppercase">Manual Entry</span>
            <div className="flex-grow border-t border-white/5"></div>
          </div>

          <form onSubmit={handleVerify} className="space-y-6">
            <div className="space-y-2">
              <label className="text-[10px] font-black uppercase ml-1 text-gray-500">Order Reference</label>
              <Input 
                placeholder="Ex: ORDER-12345" 
                value={orderId}
                onChange={(e) => setOrderId(e.target.value.toUpperCase())}
                className="bg-[#141F2D]/40 border-white/5 text-white h-14 rounded-2xl text-lg font-bold"
              />
            </div>

            <div className="space-y-2">
              <label className="text-[10px] font-black uppercase ml-1 text-gray-500">Delivery OTP</label>
              <Input 
                type="text"
                placeholder="••••••"
                maxLength={6}
                value={otp}
                onChange={(e) => setOtp(e.target.value)}
                className="bg-[#141F2D]/40 border-white/5 text-[#EFA765] h-16 rounded-2xl text-center text-3xl tracking-[0.4em] font-black"
              />
            </div>
            ...
            <Button 
              type="submit" 
              disabled={loading || !orderId || !otp}
              className="w-full bg-[#EFA765] text-[#141F2D] hover:bg-white h-16 rounded-2xl text-lg font-black transition-all shadow-xl shadow-[#EFA765]/5"
            >
